...
--SUMA
SELECT SUM(precio) FROM productos; --93295
--PROMEDIO
SELECT AVG(precio) FROM productos; --4664.750000
--PRECIO MAXIMO
SELECT MAX(precio) FROM productos; --9688
--PRECIO MINIMO 
SELECT MIN(precio) FROM productos; --298

--Cuenta la cantidad de productos con stock
--(pista: puedes combinar las funciones de agregado con where)
SELECT COUNT(en_stock) FROM productos WHERE en_stock = 't'; -- 6 

--Cuenta la cantidad de precios sobre 7000
SELECT COUNT(precio) FROM productos WHERE precio > 7000; --4

--Calcula el promedio de los productos que tienen stock
SELECT AVG(precio) FROM productos WHERE en_stock = 't'; -- 2101.33333

--GROUP BY
-- el precio maximo de cada producto por categoria
SELECT MAX(precio) FROM productos GROUP BY categoria;
SELECT categoria, MAX(precio) FROM productos GROUP BY categoria;

--regla importante, no podemos mostrar campos no agregados salvo que esten en funciones de agregado
...
--LINE 1: SELECT nombre_producto,max(precio) from productos GROUP BY c...


1. ¿Cuál es el promedio de ventas?
SELECT * FROM ventas;
SELECT AVG(precio_unitario) FROM ventas;-- 
SELECT *,(cantidad*precio_unitario) as venta FROM VENTAS;-- CREAR UN COLUMNA TEMPORAL
SELECT AVG(cantidad*precio_unitario) FROM VENTAS;-- 381.0000000000000000

2. ¿Cuál es el promedio de ventas por producto?
SELECT * FROM ventas;
SELECT producto FROM VENTAS GROUP BY producto;-- 6 rows
SELECT producto,AVG(cantidad * precio_unitario) FROM ventas group by producto;-- promedio de ventas por producto

3. ¿Cuál es el promedio de ventas por producto después del '2024-01-05'?
SELECT producto,AVG(cantidad*precio_unitario) FROM ventas WHERE fecha>'2024-01-05'GROUP BY producto;-- 5 rows
